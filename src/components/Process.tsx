    ...
    >
      <div className="flex flex-col xl:flex-row xl:items-end justify-between">
        <h2 className="mb-4 xl:mb-0 text-black text-[7.4vw] md:text-5xl xl:text-6xl font-semibold md:font-medium leading-snug md:leading-tight">
          프로젝트의 <span>ZERO</span>에서 <br /> <span>HERO</span> 그 이상까지{' '}
          <br className="md:hidden" /> 함께할 수 있습니다.
        </h2>
        <p className="lg:w-[480px] text-[15px] lg:text-lg xl:text-2xl leading-relaxed break-keep">
          다양한 분야의 경험과 지식을 융합하고 이용하여,
          <br className="md:hidden" /> 팀원들이 발견할 수 없는 부분까지 넓고 멀리 생각하기 위해
          노력합니다.
        </p>
      </div>
      <ul className="grid md:grid-cols-2 xl:grid-cols-3 mt-24 gap-x-16 gap-y-14 lg:gap-y-20">
        {sectors.map((sector, idx) => (
          <li key={idx} className="w-full">
            {sector.svg}
            <h3 className="text-2xl sm:text-3xl mb-3 sm:mb-4 mt-4 sm:mt-5 font-semibold">
              {sector.sector}
            </h3>
            <p className="text-[15px] md:text-base leading-[1.75] text-foreground/70 break-keep">
              {sector.desc}
            </p>
          </li>
        ))}
...
const sectors = [
  {
    sector: '기획',
    svg: <Svg.Hero1 className="size-16 md:size-24" />,
    desc: '데이터에 기반한 유저의 데이터와 다양한 프로덕트에 대한 사용경험을 최대한 살려 와우모먼트를 이끌어내거나 도울 수 있습니다.',
  },
  {
    sector: '리서치',
    svg: <Svg.Hero5 className="size-16 md:size-24" />,
  ...
  {
    sector: '유지보수',
    svg: <Svg.Hero6 className="size-16 md:size-24" />,
