...
const Drawer = (props: React.ComponentPropsWithoutRef<typeof DrawerPrimitives.Root>) => {
  const { setScrollAvailable } = useUiStore();
  return (
    ...
    <DrawerPortal>
      <DrawerOverlay>
        <DrawerPrimitives.Content
          ...
          className={cn(
            // base
            'fixed inset-y-2 z-50 mx-auto flex w-[95vw] flex-1 flex-col overflow-y-auto rounded-xl border p-4 shadow-lg focus:outline-none max-sm:inset-x-2 sm:inset-y-2 sm:right-2 sm:p-6',
            // border color
    ...
    return (
      ...
      >
        ...
        <DrawerPrimitives.Close asChild>
          <Button
            variant="ghost"
            className="aspect-square p-1 hover:bg-[#F7F8FA] hover:dark:bg-gray-400/10"
          >
            <LucideX className="size-6" aria-hidden="true" />
          </Button>
